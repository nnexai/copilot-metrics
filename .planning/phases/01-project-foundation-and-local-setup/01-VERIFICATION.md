...
| 4 | VS Code and Copilot CLI setup output points telemetry JSONL files at central data directory with content capture disabled. | PASS | `src/setup.js` emits current VS Code OTel keys and Copilot CLI OTel variables with content capture disabled. |
| 5 | Hook setup supports local repository and user-global modes. | PASS | `hooks install --scope local|global` writes local `.github/hooks` or `$COPILOT_HOME/hooks` / `~/.copilot/hooks`. |
...
node bin/copilot-metrics.js setup vscode --json
node bin/copilot-metrics.js setup copilot-cli --json
```
