# Requirements Archive: v0.1.1 Local Copilot Usage Tracker
...
- [x] **SETUP-06**: User can complete setup once and then run normal CLI/report commands without remembering environment variables or manual import commands.
...
- [x] **INGEST-06**: Report commands automatically import pending configured local JSONL sources before querying without double-counting records.
...
- [x] **REPORT-09**: Label reports expose input, output, cache read, cache creation, and reasoning token totals in human-readable and machine-readable output.
- [x] **REPORT-10**: Reports distinguish hook-only attribution evidence from token-bearing usage so zero-token labels are not misleading.
