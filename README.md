...
```bash
npx copilot-metrics@0.2.0 --help
npx copilot-metrics@0.2.0 init
```
...
```bash
npx copilot-metrics@0.2.0 init
npx copilot-metrics@0.2.0 paths --json
```
...
```bash
npx copilot-metrics@0.2.0 hooks preview --scope local --surface both
```

Install repo-local or user-global hook config:

```bash
npx copilot-metrics@0.2.0 hooks install --scope local --surface both
npx copilot-metrics@0.2.0 hooks install --scope global --surface both
```
...
```bash
npx copilot-metrics@0.2.0 store init
npx copilot-metrics@0.2.0 import --source vscode --file ~/.local/share/copilot-metrics/telemetry/vscode-copilot-otel.jsonl
npx copilot-metrics@0.2.0 import --source copilot-cli --file ~/.local/share/copilot-metrics/telemetry/copilot-cli-otel.jsonl
npx copilot-metrics@0.2.0 import --source copilot-session --file ~/.copilot/session-state/<session-id>/events.jsonl
npx copilot-metrics@0.2.0 import --source vscode-chat --file ~/.config/Code\ -\ Insiders/User/workspaceStorage/<workspace-id>/chatSessions/<session-id>.jsonl
npx copilot-metrics@0.2.0 import --source hooks --file ~/.local/share/copilot-metrics/hooks/copilot-hooks.jsonl
```
...
```bash
npx copilot-metrics@0.2.0 report labels
npx copilot-metrics@0.2.0 report label DEMO-12345
npx copilot-metrics@0.2.0 report label DEMO-12345 --detail
npx copilot-metrics@0.2.0 report models
npx copilot-metrics@0.2.0 report repos
npx copilot-metrics@0.2.0 report unattributed
```

Every report supports `--json`:

```bash
npx copilot-metrics@0.2.0 report labels --json
```
...
`AI Credits est.` is a local what-would-this-cost estimate derived from the strongest available pricing evidence, not a claim that the interaction was billed today. Reports distinguish trusted observed local charge evidence, VS Code displayed-credit evidence marked as `display*`, high-confidence token-price estimates, and upper-bound estimates when cache-read counts are unavailable. Some included or request-based models can appear as `0x` in Copilot while still having published per-token prices. When displayed credits and token prices make the math bounded, reports can expose inferred effective cache-read tokens as diagnostics; observed `cache_read_tokens` is not overwritten by inferred values. The project treats 1 AI Credit as $0.01 for estimates; GitHub billing remains the source of truth.
...
- Official GitHub usage report reconciliation is not included in `0.2.0`.
- Local OTLP collector mode is not included in `0.2.0`.
- Richer prompt/content capture and redaction controls are not included in `0.2.0`.
