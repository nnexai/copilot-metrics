...
```bash
npx copilot-metrics@0.2.1 --help
npx copilot-metrics@0.2.1 init
```
...
```bash
npx copilot-metrics@0.2.1 init
npx copilot-metrics@0.2.1 paths --json
```
...
```bash
npx copilot-metrics@0.2.1 hooks preview --scope local --surface both
```

Install repo-local or user-global hook config:

```bash
npx copilot-metrics@0.2.1 hooks install --scope local --surface both
npx copilot-metrics@0.2.1 hooks install --scope global --surface both
```
...
```bash
npx copilot-metrics@0.2.1 store init
npx copilot-metrics@0.2.1 import --source vscode --file ~/.local/share/copilot-metrics/telemetry/vscode-copilot-otel.jsonl
npx copilot-metrics@0.2.1 import --source copilot-cli --file ~/.local/share/copilot-metrics/telemetry/copilot-cli-otel.jsonl
npx copilot-metrics@0.2.1 import --source copilot-session --file ~/.copilot/session-state/<session-id>/events.jsonl
npx copilot-metrics@0.2.1 import --source vscode-chat --file ~/.config/Code\ -\ Insiders/User/workspaceStorage/<workspace-id>/chatSessions/<session-id>.jsonl
npx copilot-metrics@0.2.1 import --source hooks --file ~/.local/share/copilot-metrics/hooks/copilot-hooks.jsonl
```
...
```bash
npx copilot-metrics@0.2.1 report labels
npx copilot-metrics@0.2.1 report label DEMO-12345
npx copilot-metrics@0.2.1 report label DEMO-12345 --detail
npx copilot-metrics@0.2.1 report models
npx copilot-metrics@0.2.1 report repos
npx copilot-metrics@0.2.1 report unattributed
```

Every report supports `--json`:

```bash
npx copilot-metrics@0.2.1 report labels --json
```
...
- Official GitHub usage report reconciliation is not included in `0.2.1`.
- Local OTLP collector mode is not included in `0.2.1`.
- Richer prompt/content capture and redaction controls are not included in `0.2.1`.
