...
```bash
npx copilot-metrics@0.3.0 --help
npx copilot-metrics@0.3.0 init
```
...
```bash
npx copilot-metrics@0.3.0 init
npx copilot-metrics@0.3.0 paths --json
```
...
```bash
npx copilot-metrics@0.3.0 hooks preview --scope local --surface both
```

Install repo-local or user-global hook config:

```bash
npx copilot-metrics@0.3.0 hooks install --scope local --surface both
npx copilot-metrics@0.3.0 hooks install --scope global --surface both
```
...
```bash
npx copilot-metrics@0.3.0 store init
npx copilot-metrics@0.3.0 import --source vscode --file ~/.local/share/copilot-metrics/telemetry/vscode-copilot-otel.jsonl
npx copilot-metrics@0.3.0 import --source copilot-cli --file ~/.local/share/copilot-metrics/telemetry/copilot-cli-otel.jsonl
npx copilot-metrics@0.3.0 import --source copilot-session --file ~/.copilot/session-state/<session-id>/events.jsonl
npx copilot-metrics@0.3.0 import --source vscode-chat --file ~/.config/Code\ -\ Insiders/User/workspaceStorage/<workspace-id>/chatSessions/<session-id>.jsonl
npx copilot-metrics@0.3.0 import --source hooks --file ~/.local/share/copilot-metrics/hooks/copilot-hooks.jsonl
```
...
```bash
npx copilot-metrics@0.3.0 report labels
npx copilot-metrics@0.3.0 report label DEMO-12345
npx copilot-metrics@0.3.0 report label DEMO-12345 --detail
npx copilot-metrics@0.3.0 report models
npx copilot-metrics@0.3.0 report repos
npx copilot-metrics@0.3.0 report unattributed
```

Every report supports `--json`:

```bash
npx copilot-metrics@0.3.0 report labels --json
```
...
The default extractor finds Jira-style labels. To keep the built-in metadata scanning and evidence behavior but change the regex, add `labelPatterns` to the local `config.json`:

After `copilot-metrics init`, add one or more patterns:
...
Each pattern is applied to the same safe fields as the default extractor. If the regex has a capture group, the first group is used as the label; otherwise the full match is used. Single-pattern aliases are also accepted as `labelPattern` or `labelRegex`.
...
- Official GitHub usage report reconciliation is not included in `0.3.0`.
- Local OTLP collector mode is not included in `0.3.0`.
- Richer prompt/content capture and redaction controls are not included in `0.3.0`.
